...
class Minefield {
    ...
    constructor(width, height, mines = Math.floor(width * height / 5), randomizer = Math.random) {
        ...
        let getNearbyCellsTemp = (cell) => {
            ...
            if (isNotFirstRow)
                nearbyCells.push(cell - this.width); //up
            if (isNotLastRow)
                nearbyCells.push(cell + this.width); //down
            if (x > 0) //if cell isn't on first column
             {
                nearbyCells.push(cell - 1); //left
                if (isNotFirstRow)
                    nearbyCells.push(cell - this.width - 1); //up left
                if (isNotLastRow)
                    nearbyCells.push(cell + this.width - 1); //down left
            }
            if (x < this.width - 1) //if cell isn't on last column
             {
                nearbyCells.push(cell + 1); //right
                if (isNotFirstRow)
                    nearbyCells.push(cell - this.width + 1); //up right
                if (isNotLastRow)
                    nearbyCells.push(cell + this.width + 1); //down right
            }
            return nearbyCells;
        };
     ...
     */
    toMinefield2D() {
        if (this instanceof Minefield2D)
            throw new Error("This object is already an instance of Minefield2D");
    ...
    openCell(cell, firstclick = this.isNew(), { nearbyOpening = true, nearbyFlagging = true } = {}) {
        ...
        if (this[cell].isOpen == false) {
            ...
            for (let j = 0; j < nearbyCells.length; j++) {
                if (this[nearbyCells[j]].isOpen == false) {
                    closedCells++;
                    if (this[nearbyCells[j]].isFlagged)
                        flaggedCells++;
                    else
                        unflaggedCells.push(nearbyCells[j]);
                }
            }
        ...
        if (updatedCells.length >= 2 && updatedCells[0] == updatedCells[1])
            updatedCells.shift();
        return updatedCells;
    }
    /**
    ...
    isSolvableFrom(cell, restore = true) {
        ...
        if (firstClick.length <= 1 && this[firstClick[0]].mines != 0) {
            if (restore)
                this[firstClick[0]].isOpen = false;
            return false;
        }
        let updates = true;
        while (updates) {
                ...
                while (shiftUpdates) //shifting & adding phantom bombs
                 {
                    ...
                    for (let i of importantCells) {
                        ...
                        for (let k = 0; k < nearbyCells.length; k++) {
                            if (this[nearbyCells[k]].isFlagged)
                                flaggedCells++;
                            else if (this[nearbyCells[k]].isOpen == false)
                                closedCells.push(nearbyCells[k]);
                        }
    ...
    getSquareZone(begIndex, endIndex) {
        ...
        if (endCords[0] < begCords[0])
            [begCords[0], endCords[0]] = [endCords[0], begCords[0]];
        if (endCords[1] < begCords[1])
            [begCords[1], endCords[1]] = [endCords[1], begCords[1]];
        let squareZone = [];
     ...
     */
    isNew() {
        return this.forEachCell(cell => { if (cell.isOpen)
     ...
     */
    isLost() {
        return this.forEachCell(cell => { if (cell.isOpen && cell.isMine)
...
class Minefield2D extends Minefield {
    ...
    getNearbyCells([x, y], includeSelf = false) {
        ...
        let isNotLastRow = y < this.height - 1;
        if (includeSelf)
            nearbyCells.push([x, y]); //center
        if (isNotFirstRow)
            nearbyCells.push([x, y - 1]); //up
        if (isNotLastRow)
            nearbyCells.push([x, y + 1]); //down
        if (x > 0) //if cell isn't on first column
         {
            nearbyCells.push([x - 1, y]); //left
            if (isNotFirstRow)
                nearbyCells.push([x - 1, y - 1]); //up left
            if (isNotLastRow)
                nearbyCells.push([x - 1, y + 1]); //down left
        }
        if (x < this.width - 1) //if cell isn't on last column
         {
            nearbyCells.push([x + 1, y]); //right
            if (isNotFirstRow)
                nearbyCells.push([x + 1, y - 1]); //up right
            if (isNotLastRow)
                nearbyCells.push([x + 1, y + 1]); //down right
        }
        return nearbyCells;
    }
    /**
...
function validateNumber(num, min = -Infinity, max = Infinity) {
    ...
    if (num < min)
        throw new Error("Parameter value is too small");
    if (num > max)
        throw new Error("Parameter value is too big");
    return num;
}
