...
__export(src_exports, {
  ...
  isAggrExpr: () => isAggrExpr,
  isComparable: () => isComparable,
  isEmpty: () => isEmpty,
  isEvalExpr: () => isEvalExpr,
  isFlat: () => isFlat,
  isUpdateExpr: () => isUpdateExpr,
...
((Type2) => {
  ...
  function transform(value, type, callback) {
    if (!(0, import_cosmokit.isNullable)(value) && type?.inner) {
      if (Type2.isArray(type)) {
        ...
        if (type.ignoreNull && isEmpty(value))
          return null;
        return (0, import_cosmokit.mapValues)(value, (x, k) => callback(x, Type2.getInner(type, k)));
      }
    }
    return value;
  }
...
((Relation4) => {
  ...
  function parse(def, key, model, relmodel, subprimary) {
    ...
    const fields = def.fields ?? (subprimary || model.name === relmodel.name || def.type === "manyToOne" || def.type === "oneToOne" && !(0, import_cosmokit2.makeArray)(relmodel.primary).every((key2) => !relmodel.fields[key2]?.nullable) ? (0, import_cosmokit2.makeArray)(relmodel.primary).map((x) => `${key}.${x}`) : model.primary);
    const relation = {
      type: def.type,
      table: def.table ?? relmodel.name,
      fields: (0, import_cosmokit2.makeArray)(fields),
      shared,
      references: (0, import_cosmokit2.makeArray)(def.references ?? relmodel.primary),
      required: def.type !== "manyToOne" && model.name !== relmodel.name && (0, import_cosmokit2.makeArray)(fields).every((key2) => !model.fields[key2]?.nullable || (0, import_cosmokit2.makeArray)(model.primary).includes(key2))
    };
    ...
    const inverse = {
      ...
      required: relation.type !== "oneToMany" && relation.references.every((key2) => !relmodel.fields[key2]?.nullable || (0, import_cosmokit2.makeArray)(relmodel.primary).includes(key2))
    };
    if (inverse.required)
...
var Model = class {
  ...
  avaiableFields() {
    return (0, import_cosmokit2.filterKeys)(this.fields, (_, field) => Field.available(field));
  }
  getType(key) {
...
var Eval2 = /* @__PURE__ */ __name((key, value, type) => (0, import_cosmokit3.defineProperty)((0, import_cosmokit3.defineProperty)({ ["$" + key]: value }, kExpr, true), Type.kType, type), "Eval");
...
(0, import_cosmokit3.defineProperty)(Eval2, "length", unary("length", (expr, table) => Array.isArray(table) ? table.map((data) => executeAggr(expr, data)).length : Array.from(executeEval(table, expr)).length, Type.Number));
...
Eval2.object = (fields) => {
  if (fields.$model) {
    ...
    return Eval2("object", fields, Type.Object((0, import_cosmokit3.mapValues)(fields, (value) => Type.fromTerm(value))));
  }
  return Eval2("object", fields, Type.Object((0, import_cosmokit3.mapValues)(fields, (value) => Type.fromTerm(value))));
};
...
function isFlat(value) {
  return !value || typeof value !== "object" || isEvalExpr(value) || Object.keys(value).length === 0 || Array.isArray(value) || value instanceof Date || value instanceof RegExp || import_cosmokit4.Binary.isSource(value);
}
...
function isEmpty(value) {
  if ((0, import_cosmokit4.isNullable)(value))
    return true;
...
var Executable = class {
  ...
  constructor(driver, payload) {
    ...
    (0, import_cosmokit5.defineProperty)(this, "driver", driver);
    (0, import_cosmokit5.defineProperty)(this, "model", driver.model(this.table));
    (0, import_cosmokit5.defineProperty)(this, "row", createRow(this.ref, {}, "", this.model));
  }
  resolveQuery(query = {}) {
...
var Selection = class _Selection extends Executable {
  ...
  join(name, selection, callback = () => Eval2.and(), optional = false) {
    ...
    if (optional) {
      return this.driver.database.join({ [this.ref]: this, [name]: selection }, (t) => callback(t[this.ref], t[name]), { [this.ref]: false, [name]: true }).project({ ...fields, [name]: (row) => Eval2.ignoreNull(Eval2.object((0, import_cosmokit5.mapValues)(joinFields, (x) => x(row)))) });
    } else {
      return this.driver.database.join({ [this.ref]: this, [name]: selection }, (t) => callback(t[this.ref], t[name])).project({ ...fields, [name]: (row) => Eval2.ignoreNull(Eval2.object((0, import_cosmokit5.mapValues)(joinFields, (x) => x(row)))) });
    }
  }
  _action(type, ...args) {
  ...
  evaluate(callback) {
    ...
    if (isAggrExpr(expr))
      (0, import_cosmokit5.defineProperty)(expr, Type.kType, Type.Array(Type.fromTerm(expr)));
  ...
  async execute(cursor) {
    ...
    if (!cursor.fields)
      return rows;
    return rows.map((row) => {
      return (0, import_cosmokit5.filterKeys)(row, (key) => {
...
var Database = class _Database extends import_cordis.Service {
  ...
  extend(name, fields, config = {}) {
    ...
    if ((0, import_cosmokit6.makeArray)(model.primary).every((key) => key in fields)) {
      (0, import_cosmokit6.defineProperty)(model, "ctx", this.ctx);
    }
    Object.entries(fields).forEach(([key, def]) => {
      if (!Relation.Type.includes(def.type))
        return;
      const subprimary = !def.fields && (0, import_cosmokit6.makeArray)(model.primary).includes(key);
    ...
    if (Array.isArray(model.primary) && model.primary.every((key) => model.fields[key]?.relation)) {
      model.primary = (0, import_cosmokit6.deduplicate)(model.primary.map((key) => model.fields[key].relation.fields).flat());
    }
    this.prepareTasks[name] = this.prepare(name);
  ...
  _parseField(field, transformers = [], setInitial, setField) {
    if (field === "object") {
      ...
      const res = Type.Object((0, import_cosmokit6.mapValues)(inner, (x, k) => this.parseField(x, transformers, (value) => initial[k] = value)));
  ...
  parseField(field, transformers = [], setInitial, setField) {
    ...
    if (typeof field === "object" && field.load && field.dump) {
      if (type.inner)
        type = Type.fromField(this.define({ ...(0, import_cosmokit6.omit)(midfield, ["load", "dump"]), type }));
  ...
  select(table, query, include) {
    ...
    if (include)
      include = (0, import_cosmokit6.filterKeys)(include, (key) => !!modelFields[key]?.relation);
    ...
    sel.query = (0, import_cosmokit6.omit)(sel.query, Object.keys(include ?? {}));
    if (Object.keys(sel.query.$not ?? {}).length) {
      sel.query.$not = (0, import_cosmokit6.omit)(sel.query.$not, Object.keys(include ?? {}));
  ...
  join(tables, query = (...args) => Eval2.and(), optional) {
    ...
    let sels = (0, import_cosmokit6.mapValues)(tables, (t) => {
  ...
  async set(table, query, update) {
    ...
    const primary = (0, import_cosmokit6.makeArray)(sel.model.primary);
    ...
    if (relations.length) {
      return await this.ensureTransaction(async (database) => {
        ...
        let baseUpdate = (0, import_cosmokit6.omit)(rawupdate(sel.row), relations.map(([key]) => key));
  ...
  async create(table, data) {
    ...
    if (!this.hasRelation(table, data)) {
      ...
      if (!autoInc) {
        const keys = (0, import_cosmokit6.makeArray)(primary);
  ...
  async upsert(table, upsert, keys) {
    ...
    keys = (0, import_cosmokit6.makeArray)(keys || sel.model.primary);
  ...
  async transact(callback) {
    ...
    return initialTask.catch(import_cosmokit6.noop).finally(() => Promise.all(finalTasks));
  }
  async stopAll() {
  ...
  async createOrUpdate(table, data, upsert = true) {
    ...
    for (const key of [...tasks]) {
      if (!key) {
        const { primary, autoInc } = sel.model;
        const keys = (0, import_cosmokit6.makeArray)(primary);
        if (keys.some((key2) => (0, import_cosmokit6.isNullable)(getCell(data, key2)))) {
          if (!autoInc) {
            throw new Error("missing primary key");
          } else {
            delete data[primary];
        ...
        if (upsert) {
          await sel._action("upsert", [sel.model.format((0, import_cosmokit6.omit)(data, tasks))], keys).execute();
        } else {
          Object.assign(data, await sel._action("create", sel.model.create((0, import_cosmokit6.omit)(data, tasks))).execute());
        }
        continue;
      }
      ...
      if (relation.type === "oneToOne") {
        if (value.$literal) {
          ...
          (0, import_cosmokit6.remove)(tasks, key);
        ...
        if (value.$create || Array.isArray(value)) {
          for (const item of (0, import_cosmokit6.makeArray)(value.$create ?? value)) {
        ...
        if (value.$upsert) {
          await this.upsert(relation.table, (0, import_cosmokit6.makeArray)(value.$upsert).map((r) => ({
            ...Object.fromEntries(relation.references.map((k, i) => [k, getCell(data, relation.fields[i])])),
            ...r
          })));
        }
        ...
        if (value.$upsert) {
          const upsert2 = (0, import_cosmokit6.makeArray)(value.$upsert).map((r) => ({
        ...
        if (value.$connect) {
          for (const item of (0, import_cosmokit6.makeArray)(value.$connect)) {
  ...
  async processRelationUpdate(table, row, key, value) {
    ...
    if (relation.type === "oneToOne") {
      ...
      if (value.$set || typeof value === "function") {
        for (const setexpr of (0, import_cosmokit6.makeArray)(value.$set ?? value)) {
      ...
      if (value.$create) {
        for (const item of (0, import_cosmokit6.makeArray)(value.$create)) {
          await this.createOrUpdate(relation.table, {
            ...Object.fromEntries(relation.references.map((k, i) => [k, getCell(row, relation.fields[i])])),
            ...item
          });
      ...
      if (value.$set) {
        for (const setexpr of (0, import_cosmokit6.makeArray)(value.$set)) {
          const [query, update2] = setexpr.update ? [setexpr.where, setexpr.update] : [{}, setexpr];
      ...
      if (value.$upsert) {
        ...
        await this.upsert(assocTable, (0, import_cosmokit6.makeArray)(value.$upsert).map((r) => ({
          ...Object.fromEntries(shared.map(([k, v]) => [k, getCell(row, v.field)])),
          ...Object.fromEntries(fields.map((k, i) => [k, row[relation.fields[i]]])),
          ...Object.fromEntries(references.map((k, i) => [k, r[relation.references[i]]]))
        })));
    ...
    if (Object.keys(update).length) {
      await this.set(table, (0, import_cosmokit6.pick)(model.format(row), (0, import_cosmokit6.makeArray)(model.primary)), update);
    }
  }
...
var Driver = class {
  constructor(ctx, config) {
    ...
    ctx.on("ready", async () => {
      ...
      (0, import_cosmokit7.defineProperty)(database, "ctx", ctx);
      database._driver = this;
      database[import_cordis2.Service.tracker] = {
        associate: "database",
        property: "ctx"
      };
    ...
    ctx.on("dispose", async () => {
      (0, import_cosmokit7.remove)(ctx.model.drivers, this);
      await this.stop();
    });
  }
  static {
  ...
  model(table) {
    ...
    if (table instanceof Selection) {
      ...
      if (table.args[0].fields) {
        model2.fields = (0, import_cosmokit7.mapValues)(table.args[0].fields, (expr) => ({
          type: Type.fromTerm(expr)
        }));
      } else {
        model2.fields = (0, import_cosmokit7.mapValues)(table.model.fields, (field) => ({
          type: Type.fromField(field)
        }));
      }
      return model2;
...
var queryOperators = {
  ...
  // existence
  $exists: (query, data) => query !== (0, import_cosmokit8.isNullable)(data),
  // comparison
...
function executeFieldQuery(query, data) {
  ...
  } else if ((0, import_cosmokit8.isNullable)(query)) {
    return (0, import_cosmokit8.isNullable)(data);
  }
