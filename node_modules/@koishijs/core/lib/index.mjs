...
// src/context.ts
import { defineProperty as defineProperty6, Time as Time3 } from "cosmokit";
import { HTTP, Schema as Schema5 } from "@satorijs/core";
...
import { h as h3, Schema as Schema2, Universal as Universal3 } from "@satorijs/core";

// src/command/command.ts
import { camelize, isNullable, remove } from "cosmokit";
import { coerce as coerce2 } from "@koishijs/utils";
import { Logger, Schema } from "@satorijs/core";
...
import { defineProperty as defineProperty2, Time } from "cosmokit";
import { h as h2 } from "@satorijs/core";
...
var Processor = class {
  constructor(ctx) {
    this.ctx = ctx;
    defineProperty2(this, Context.current, ctx);
  ...
  _executeMatcher(session, matcher) {
    ...
    session.response = async () => {
      ...
      return h2.normalize(output, params.map((source) => source ? h2.parse(source) : ""));
    };
  }
...
var Commander = class {
  constructor(ctx, config = {}) {
    this.ctx = ctx;
    this.config = config;
    defineProperty3(this, Context.current, ctx);
    ctx.plugin(validate);
    ...
    ctx.on("interaction/command", (session) => {
      if (session.event?.argv) {
        ...
        defineProperty3(session, "argv", ctx.bail("before-parse", session.content, session));
        if (!session.argv) {
    ...
    ctx.before("attach", (session) => {
      ...
      defineProperty3(session, "argv", ctx.bail("before-parse", content, session));
      if (!session.argv)
        return;
      session.argv.root = true;
      session.argv.session = session;
    ...
    ctx.schema.extend("command-option", Schema2.object({
      permissions: Schema2.array(String).role("perms").default(["authority:0"]).description("权限继承。"),
      dependencies: Schema2.array(String).role("perms").description("权限依赖。")
    }), 1e3);
    ctx.on("ready", () => {
      const bots = ctx.bots.filter((v) => v.status === Universal3.Status.ONLINE && v.updateCommands);
      bots.forEach((bot) => this.updateCommands(bot));
    });
    ctx.on("bot-status-updated", async (bot) => {
      if (bot.status !== Universal3.Status.ONLINE || !bot.updateCommands)
        return;
      this.updateCommands(bot);
    });
    this.domain("el", (source) => h3.parse(source), { greedy: true });
    this.domain("elements", (source) => h3.parse(source), { greedy: true });
    this.domain("string", (source) => h3.unescape(source));
    this.domain("text", (source) => h3.unescape(source), { greedy: true });
    this.domain("rawtext", (source) => h3("", h3.parse(source)).toString(true), { greedy: true });
    ...
    this.domain("date", (source, session) => {
      const timestamp = Time2.parseDate(source);
      if (+timestamp)
        return timestamp;
  ...
  _resolvePrefixes(session) {
    ...
    return result.map((source) => h3.escape(source)).sort().reverse();
  }
  available(session) {
...
import { h as h4, Logger as Logger2, Schema as Schema3 } from "@satorijs/core";
...
var I18n = class {
  ...
  *set(locale, prefix, value) {
    if (typeof value === "object" && value && !prefix.includes("@")) {
      ...
      if (!isNullable3(dict[path2]) && !locale.startsWith("$") && dict[path2] !== value) {
  ...
  _render(value, params, locale) {
    ...
    if (typeof value !== "string") {
      ...
      return [h4.text(render(value, params, locale))];
    }
    return h4.parse(value, params);
  }
  /** @deprecated */
  ...
  render(locales, paths, params) {
    ...
    return [h4.text(paths[0])];
  }
};
((I18n2) => {
  I18n2.Config = Schema3.object({
    locales: Schema3.array(String).role("table").default(["zh-CN", "en-US", "fr-FR", "ja-JP", "de-DE", "ru-RU"]).description("可用的语言列表。按照回退顺序排列。"),
    output: Schema3.union([
      Schema3.const("prefer-user").description("优先使用用户语言"),
      Schema3.const("prefer-channel").description("优先使用频道语言")
...
import { isNullable as isNullable4, makeArray as makeArray2 } from "cosmokit";
import { h as h5, Logger as Logger3 } from "@satorijs/core";
import { executeEval, isEvalExpr } from "minato";
var logger3 = new Logger3("session");
...
var KoishiSession = class {
  ...
  async sendQueued(content, delay) {
    const text = h5.normalize(content).join("");
    if (!text)
      return;
    if (isNullable4(delay)) {
  ...
  async withScope(scope, callback) {
    ...
    try {
      ...
      return h5.transform(result, {
        i18n: (params, children) => h5.i18n({
          ...params,
  ...
  i18n(path2, params = {}) {
    ...
    const paths = makeArray2(path2).map((path3) => this.resolveScope(path3));
    ...
    if (argv.tokens) {
      for (const arg of argv.tokens) {
        ...
        for (let i = 0; i < inters.length; ++i) {
          ...
          const transformed = await this.transform(h5.normalize(execution));
    ...
    return this.withScope(`commands.${command.name}.messages`, async () => {
      ...
      if (!shouldEmit) {
        if (typeof result === "string")
          return result;
        return h5(null, result).toString();
      }
...
// src/permission.ts
import { Logger as Logger4 } from "@satorijs/core";
import { defineProperty as defineProperty4, remove as remove2 } from "cosmokit";
var logger4 = new Logger4("app");
var Permissions = class {
  constructor(ctx) {
    this.ctx = ctx;
    defineProperty4(this, Context.current, ctx);
  ...
  define(pattern, options) {
    ...
    return this.ctx.effect(() => {
      ...
      return () => remove2(this.store, entry);
    });
  }
...
import { Adapter, Bot as Bot2 } from "@satorijs/core";
defineProperty5(Bot2, "filter", false);
defineProperty5(Adapter, "filter", false);
var KoishiBot = class {
  static {
...
// src/schema.ts
import { remove as remove3 } from "cosmokit";
import { Schema as Schema4 } from "@satorijs/core";
Schema4.dynamic = /* @__PURE__ */ __name(function dynamic(name) {
  return Schema4.any().role("dynamic", { name });
}, "dynamic");
Schema4.filter = /* @__PURE__ */ __name(function filter() {
  return Schema4.any().role("filter");
}, "filter");
Schema4.computed = /* @__PURE__ */ __name(function computed(inner, options = {}) {
  return Schema4.union([
    Schema4.from(inner),
    Schema4.object({
      $switch: Schema4.object({
        branches: Schema4.array(Schema4.object({
          case: Schema4.any(),
          then: Schema4.from(inner)
        })),
        default: Schema4.from(inner)
      })
    }).hidden(),
    Schema4.any().hidden()
  ]).role("computed", options);
}, "computed");
Schema4.path = /* @__PURE__ */ __name(function path(options = {}) {
  return Schema4.string().role("path", options);
}, "path");
Schema4.prototype.computed = /* @__PURE__ */ __name(function computed2(options = {}) {
  return Schema4.computed(this, options).default(this.meta.default);
}, "computed");
var kSchemaOrder = Symbol("schema-order");
var SchemaService = class {
  constructor(ctx) {
    this.ctx = ctx;
    this.extend("intercept.http", Schema4.object({
      timeout: Schema4.natural().role("ms").description("等待连接建立的最长时间。"),
      proxyAgent: Schema4.string().description("使用的代理服务器地址。"),
      keepAlive: Schema4.boolean().description("是否保持连接。")
    }));
  }
  static {
  ...
  extend(name, schema, order = 0) {
    ...
    caller?.on("dispose", () => {
      remove3(target.list, schema);
      this.ctx.emit("internal/schema", name);
    });
  }
  get(name) {
    return this._data[name] ||= Schema4.intersect([]);
  }
  set(name, schema) {
    const caller = this[Context.current];
...
// src/context.ts
import { Adapter as Adapter2, Bot as Bot3, Element, h as h6, HTTP as HTTP2, Logger as Logger5, MessageEncoder, Messenger, Quester, Schema as Schema6, segment, Universal as Universal5, z } from "@satorijs/core";
...
((Context3) => {
  Context3.Config = Schema5.intersect([
    Schema5.object({})
  ]);
})(Context || (Context = {}));
defineProperty6(Context.Config, "Basic", Schema5.object({
  prefix: Schema5.array(Schema5.string().default("")).default([""]).role("table").computed().description("指令前缀字符构成的数组。将被用于指令的匹配。"),
  prefixMode: Schema5.union([
    Schema5.const("auto").description("默认：当存在称呼时允许无前缀触发。"),
    Schema5.const("strict").description("严格：只有在指令前缀匹配时才允许触发。")
  ]).experimental().role("radio").default("auto").description("指令前缀匹配模式。"),
  nickname: Schema5.array(String).role("table").computed().description("机器人昵称构成的数组。将被用于指令的匹配。"),
  autoAssign: Schema5.boolean().default(true).computed().description("当获取不到频道数据时，是否使用接受者作为受理人。"),
  autoAuthorize: Schema5.natural().default(1).computed().description("当获取不到用户数据时默认使用的权限等级。"),
  minSimilarity: Schema5.percent().default(1).description("用于模糊匹配的相似系数，应该是一个 0 到 1 之间的数值。数值越高，模糊匹配越严格。设置为 1 可以完全禁用模糊匹配。")
}).description("基础设置"));
defineProperty6(Context.Config, "I18n", I18n.Config);
defineProperty6(Context.Config, "Delay", Schema5.object({
  character: Schema5.natural().role("ms").default(0).description("调用 `session.sendQueued()` 时消息间发送的最小延迟，按前一条消息的字数计算。"),
  message: Schema5.natural().role("ms").default(0.1 * Time3.second).description("调用 `session.sendQueued()` 时消息间发送的最小延迟，按固定值计算。"),
  cancel: Schema5.natural().role("ms").default(0).description("调用 `session.cancelQueued()` 时默认的延迟。"),
  broadcast: Schema5.natural().role("ms").default(0.5 * Time3.second).description("调用 `bot.broadcast()` 时默认的延迟。"),
  prompt: Schema5.natural().role("ms").default(Time3.minute).description("调用 `session.prompt()` 时默认的等待时间。")
}));
defineProperty6(Context.Config, "Advanced", Schema5.object({
  maxListeners: Schema5.natural().default(64).description("每种监听器的最大数量。如果超过这个数量，Koishi 会认定为发生了内存泄漏，将产生一个警告。")
}).description("高级设置"));
Context.Config.list.push(Context.Config.Basic);
Context.Config.list.push(Schema5.object({
  i18n: I18n.Config
}));
Context.Config.list.push(Schema5.object({
