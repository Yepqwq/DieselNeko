...
class BALogo {
    ...
    transparentBg;
    font;
    fontFamily = 'RoGSanSrfStd-Bd, GlowSansSC-Normal-Heavy_diff, apple-system, BlinkMacSystemFont, Segoe UI, Helvetica, Arial, PingFang SC, Hiragino Sans GB, Microsoft YaHei, sans-serif';
    hollowPath = [
        [284, 136],
        [321, 153],
        [159, 410],
    ...
    async loadFonts(text) {
        await document.fonts.load(this.font, text);
    }
    async draw({ textL, textR }) {
        ...
        ctx.resetTransform();
        // Halo
        ctx.drawImage(halo, this.canvasWidthL - canvas.height / 2 + this.graphOffset.X, this.graphOffset.Y, this.canvasHeight, this.canvasHeight);
        // Text R
        ...
        ctx.globalCompositeOperation = 'source-over';
        // Cross
        ctx.drawImage(cross, this.canvasWidthL - canvas.height / 2 + this.graphOffset.X, this.graphOffset.Y, this.canvasHeight, this.canvasHeight);
        // output
        ...
        if (this.textWidthL + this.paddingX < this.canvasWidth / 2 &&
            ...
            ctx2.drawImage(canvas, this.canvasWidth / 2 - this.textWidthL - this.paddingX, 0, this.textWidthL + this.textWidthR + this.paddingX * 2, canvas.height, 0, 0, this.textWidthL + this.textWidthR + this.paddingX * 2, canvas.height);
        }
        else if (this.textWidthL + this.paddingX >= this.canvasWidth / 2) {
            ctx2.drawImage(canvas, 0, 0, this.textWidthL + this.textWidthR + this.paddingX * 2, canvas.height, 0, 0, this.textWidthL + this.textWidthR + this.paddingX * 2, canvas.height);
        }
        else if (this.textWidthR + this.paddingX >= this.canvasWidth / 2) {
            ctx2.drawImage(canvas, this.canvasWidth / 2 - this.textWidthL - this.paddingX, 0, canvas.width, canvas.height, 0, 0, canvas.width, canvas.height);
        }
        else {
