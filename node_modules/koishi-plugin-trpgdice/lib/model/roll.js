...
exports.rollCoin = exports.dndChar = exports.rollCheck = exports.roll = exports.SanpshotRollRegExp = exports.RollRegExp = void 0;
const data_1 = require("./data");
const dice_1 = require("./dice");
exports.RollRegExp = /^([0-9]{1,3}R)?([0-9]{1,3})?d([0-9]{1,4})?([\+-]([0-9]{1,3}d[0-9]{1,4}|[0-9]{1,3}))*$/;
exports.SanpshotRollRegExp = /^.r(([0-9]{1,3}R)?([0-9]{1,3})?d([0-9]{1,4})?)?([\+-]([0-9]{1,3}d[0-9]{1,4}|[0-9]{1,3}))*( (.+))?$/;
// 复杂掷骰
function roll(session, inputexp, inputreason) {
    let exp = inputexp;
    let reason = inputreason;
    if (!session.content.match(exports.SanpshotRollRegExp)) {
        if (!inputexp?.match(exports.RollRegExp)) {
    ...
    for (let i = 0; i < exp.length; i++) {
        ...
        // 超出部件限制直接返回，不再解析
        if (parts2Analyze.length > (0, data_1.getConfig)(session).partsMax) {
            return session.text(".overflow", (0, data_1.getConfig)(session));
        }
    }
    // 从正则解析第一个部件
    ...
    let faces = Number(firstPartArgs[6]) || (0, data_1.getConfig)(session).defaultDiceFices;
    if (times > (0, data_1.getConfig)(session).timesMax || faces > (0, data_1.getConfig)(session).facesMax) {
        return session.text(".overflow", (0, data_1.getConfig)(session));
    }
    parts[0] = {
        flag: 1,
    ...
    let answer = {
        player: (0, data_1.getNickname)(session),
...
function rollAllParts(parts) {
    ...
    for (let p of parts) {
        if (p.type == "bouns") {
            ...
            const dice = new dice_1.Dice(p.toRoll.times, p.toRoll.faces);
...
// coc检定
function rollCheck(session, attribute, reason) {
    if (attribute < 1 || attribute > 99)
        return session.text(".overflow");
    const dice = new dice_1.Dice(1, 100);
    const answer = {
        player: (0, data_1.getNickname)(session),
        reason: reason,
        result: dice.sum
    ...
    switch (true) {
        case dice.sum <= (0, data_1.getGreatSuccessNum)(session):
...
exports.rollCheck = rollCheck;
function dndChar(session, num = 1) {
    let result = session.text(".result", { player: (0, data_1.getNickname)(session) });
    for (let i = 0; i < num; i++) {
        result += "\n";
        let attr = new Array(6).fill(1).map(a => dnd4d6());
        result += session.text(".part", attr) + "\n";
    }
    return result;
}
exports.dndChar = dndChar;
function dnd4d6() {
    let dice = new dice_1.Dice(4, 6);
    return dice.sum - dice.result.sort((a, b) => a - b)[0];
}
function rollCoin(session) {
    const dice = new dice_1.Dice(1, 2);
    const answer = {
        player: (0, data_1.getNickname)(session)
    };
    if (dice.sum == 1) {
        return session.text(".tail", answer);
    }
    else {
        return session.text(".head", answer);
    }
}
exports.rollCoin = rollCoin;
