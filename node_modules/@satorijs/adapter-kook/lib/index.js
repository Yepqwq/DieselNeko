...
function encodeBit(value) {
  return (0, import_satori.isNullable)(value) ? value : value ? 1 : 0;
}
...
function transformCardElement(data) {
  ...
  return (0, import_satori.h)(type, attrs, children.map(transformCardElement));
}
__name(transformCardElement, "transformCardElement");
function adaptMessageMeta(base, data, message = {}, payload = message) {
  if (base.type === 1 /* text */) {
    message.content = base.content.replace(/@(.+?)#(\d+)/, (_, name, id) => (0, import_satori.h)("at", { id, name }).toString()).replace(/@全体成员/, () => (0, import_satori.h)("at", { type: "all" }).toString()).replace(/@在线成员/, () => (0, import_satori.h)("at", { type: "here" }).toString()).replace(/@role:(\d+);/, (_, role) => (0, import_satori.h)("at", { role }).toString()).replace(/#channel:(\d+);/, (_, id) => import_satori.h.sharp(id).toString());
    message.elements = import_satori.h.parse(message.content);
  } else if (base.type === 2 /* image */) {
    const element = (0, import_satori.h)("img", { src: base.content, file: data.attachments?.name });
...
var WsClient = class extends import_satori2.Adapter.WsClient {
  ...
  heartbeat() {
    if (!this.socket || this.bot.status !== import_satori2.Universal.Status.ONLINE) {
...
((WsClient2) => {
  WsClient2.Options = import_satori2.Schema.intersect([
    import_satori2.Schema.object({
      protocol: import_satori2.Schema.const("ws").required(process.env.KOISHI_ENV !== "browser"),
...
var HttpServer = class extends import_satori3.Adapter {
  static {
...
((HttpServer2) => {
  HttpServer2.Options = import_satori3.Schema.object({
    protocol: import_satori3.Schema.const("http").required(),
    token: import_satori3.Schema.string().description("机器人令牌。").role("secret").required(),
    verifyToken: import_satori3.Schema.string().description("验证令牌。").role("secret").required(),
...
var KookMessageEncoder = class extends import_satori4.MessageEncoder {
  static {
...
((KookMessageEncoder2) => {
  KookMessageEncoder2.Config = import_satori4.Schema.object({
...
var KookBot = class extends import_satori5.Bot {
  ...
  async createDirectChannel(userId) {
    ...
    return { id: code, type: import_satori5.Universal.Channel.Type.DIRECT };
  }
...
((KookBot2) => {
  KookBot2.Config = import_satori5.Schema.intersect([
    import_satori5.Schema.object({
      protocol: process.env.KOISHI_ENV === "browser" ? import_satori5.Schema.const("ws").default("ws") : import_satori5.Schema.union(["http", "ws"]).description("选择要使用的协议。").required()
    }),
