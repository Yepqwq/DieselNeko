...
var sanitizeCode = /* @__PURE__ */ __name((val) => val.replace(/(?<=`)(?=`)/g, "​"), "sanitizeCode");
...
var types = {
  ...
  natural: ApplicationCommand2.OptionType.INTEGER,
  bigint: ApplicationCommand2.OptionType.INTEGER,
...
var decodeArgv = /* @__PURE__ */ __name((data, command) => {
  const result = { name: command.name, arguments: [], options: {} };
  const options = data.options;
  if (!options)
    return result;
  const dataChild = options[0];
  if (dataChild && // eslint-disable-next-line operator-linebreak
  (dataChild.type === ApplicationCommand2.OptionType.SUB_COMMAND || dataChild.type === ApplicationCommand2.OptionType.SUB_COMMAND_GROUP)) {
...
function encodeCommandOptions(cmd) {
  ...
  if (cmd.children.length) {
    result.push(...cmd.children.map((child) => ({
      ...
      description_localizations: (0, import_core2.pick)(child.description, Locale3)
    })));
  } else {
    ...
    for (const option of cmd.options) {
      result.push({
        ...
        required: false,
        min_value: option.type === "posint" ? 1 : option.type === "natural" ? 0 : void 0
      });
    }
  }
